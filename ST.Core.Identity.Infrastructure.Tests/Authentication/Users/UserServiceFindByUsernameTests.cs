...
namespace ST.Core.Identity.Infrastructure.Tests.Authentication.Users
{
    ...
    {
        [Fact]
        public async Task FindByUsernameAsync_ReturnsUser_WhenUserFound()
        {
            ...
            var result = await service.FindByUsernameAsync(username);
        ...
        [Fact]
        public async Task FindByUsernameAsync_ReturnsNull_WhenUserManagerThrows()
        {
        ...
        public async Task FindByUsernameAsync_ThrowsForInvalidArguments(string? username, Type expectedException)
        {
            var userManager = FindByNameAsyncMock.WithNull();
            var logger = new Mock<ILogger<AuthenticationUserService<TestUser>>>();
            var service = new TestUserIdentityService(userManager.Object, logger.Object);

            await Assert.ThrowsAsync(expectedException, () => service.FindByUsernameAsync(username!));
        }
    }
}