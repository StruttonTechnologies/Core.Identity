...
namespace StruttonTechnologies.Core.Identity.Handler.Tests.Authorization.Roles
{
    ...
    public class RemoveRoleCommandHandlerTests : CoordinatorHandlerTestBase
    {
        [Fact]
        public async Task Handle_WhenUserExists_RemovesRole()
        {
            RemoveRoleCommand request = new(TestUser.Id.ToString(), "Admin");
            UserManagerMock.Setup(x => x.FindByIdAsync(request.UserId)).ReturnsAsync(TestUser);
            UserManagerMock.Setup(x => x.RemoveFromRoleAsync(TestUser, request.RoleName)).ReturnsAsync(IdentityResult.Success);

            object sut = InternalHandlerFactory.Create("StruttonTechnologies.Core.Identity.Coordinator.Authorization.Handlers.RemoveRoleCommandHandler`2", UserManagerMock.Object);
