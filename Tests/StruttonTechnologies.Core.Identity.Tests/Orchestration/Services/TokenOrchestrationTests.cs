...
namespace StruttonTechnologies.Core.Identity.Tests.Orchestration.Services;
...
public class TokenOrchestrationTests
{
    ...
    [Fact]
    public async Task GenerateRefreshTokenAsync_CallsJwtManager()
    {
        Guid userId = Guid.NewGuid();
        string username = "stub.user";

        _mockJwtManager
            .Setup(m => m.GenerateRefreshTokenAsync(userId, username, It.IsAny<CancellationToken>()))
            .ReturnsAsync("refresh-token");

        string result = await _mockService.GenerateRefreshTokenAsync(userId, username, TestContext.Current.CancellationToken);

        Assert.Equal("refresh-token", result);
        _mockJwtManager.Verify(m => m.GenerateRefreshTokenAsync(userId, username, It.IsAny<CancellationToken>()), Times.Once);
    }
