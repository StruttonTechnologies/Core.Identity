namespace StruttonTechnologies.Core.Identity.Tests.Handlers.Utilities;
...
internal static class InternalHandlerFactory
{
    public static object Create(string fullTypeName, params object[] arguments)
    {
        ...
        if (arity == 1)
        {
            constructedType = openType.MakeGenericType(typeof(Stub.Entities.StubUser));
        }
        else if (arity == 2)
        {
            constructedType = openType.MakeGenericType(typeof(Stub.Entities.StubUser), typeof(Guid));
        }
